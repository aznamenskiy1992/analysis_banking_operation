...
def get_income(operation: pd.DataFrame) -> str:
    ...
    return json.dumps({"income": {"total_amount": total_amount, "main": income_by_categories}}, indent=4)
...
def get_currency_rates(currencies: list) -> str:
    ...
    for currency in currencies:
        ...
        payload = {}
        headers = {"apikey": currency_data_api_key}
...
def get_stock_prices(stocks: list) -> str:
    ...
    try:
        ...
        stock_prices.extend(
            [
                {"stock": content["data"][i]["symbol"], "price": content["data"][i]["close"]}
                for i in range(len(content["data"]))
                if "symbol" in content["data"][i] and "close" in content["data"][i]
            ]
        )
