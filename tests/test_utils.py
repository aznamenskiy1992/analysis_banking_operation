...
def test_get_expenses_for_get_expenses(get_data_for_get_expenses):
    ...
    result = get_expenses(get_data_for_get_expenses)

    assert json.loads(result) == {
        "expenses": {
            "total_amount": 16613,
            "main": [
                {"category": "Дом и ремонт", "amount": 3250},
                {"category": "Супермаркеты", "amount": 681},
                {"category": "Ж/д билеты", "amount": 330},
                {"category": "Фастфуд", "amount": 296},
                {"category": "Связь", "amount": 225},
                {"category": "Каршеринг", "amount": 170},
                {"category": "Различные товары", "amount": 150},
                {"category": "Остальное", "amount": 260},
            ],
            "transfers_and_cash": [{"category": "Наличные", "amount": 8000}, {"category": "Переводы", "amount": 3250}],
        }
    }
...
def test_not_have_expenses_for_get_expenses(get_data_for_get_expenses, caplog):
    ...
    result = get_expenses(get_data_for_get_expenses[-4:])
...
def test_less_7_categories_for_get_expenses(get_data_for_get_expenses):
    ...
    result = get_expenses(get_data_for_get_expenses[-6:])

    assert json.loads(result) == {
        "expenses": {
            "total_amount": 11595,
            "main": [{"category": "Связь", "amount": 225}, {"category": "Цветы", "amount": 120}],
            "transfers_and_cash": [{"category": "Наличные", "amount": 8000}, {"category": "Переводы", "amount": 3250}],
        }
    }
...
def test_not_have_cash_category_for_get_expenses(get_data_for_get_expenses):
    ...
    result = get_expenses(get_data_for_get_expenses[-2:])

    assert json.loads(result) == {
        "expenses": {
            "total_amount": 3250,
            "main": [],
            "transfers_and_cash": [{"category": "Переводы", "amount": 3250}],
        }
    }
...
def test_not_have_transfers_category_for_get_expenses(get_data_for_get_expenses):
    ...
    result = get_expenses(get_data_for_get_expenses[-4:-2])

    assert json.loads(result) == {
        "expenses": {
            "total_amount": 8000,
            "main": [],
            "transfers_and_cash": [{"category": "Наличные", "amount": 8000}],
        }
    }
...
def test_get_income_for_get_income(get_data_for_get_income):
    ...
    assert json.loads(result) == {
        "income": {
            "total_amount": 9200,
            "main": [
                {"category": "Пополнение через Газпромбанк", "amount": 8000},
                {"category": "Пополнение через Сбер", "amount": 1200},
            ],
        }
    }
...
def test_not_have_income_for_get_income(get_data_for_get_expenses, caplog):
    ...
    assert json.loads(result) == {"income": {"total_amount": 0, "main": []}}
...
def test_get_currency_rate_for_get_currency_rates(mock_get, get_currency_response_for_get_currency_rates):
    ...
    assert json.loads(result) == {
        "currency_rates": [{"currency": "USD", "rate": 78.918179}, {"currency": "EUR", "rate": 90.000}]
    }
...
def test_get_stock_prices_for_get_stock_prices(mock_get, get_data_for_get_stock_prices):
    ...
    assert json.loads(result) == {
        "stock_prices": [{"stock": "AAPL", "price": 213.55}, {"stock": "AMZN", "price": 320.55}]
    }
